...
export class EditorJsAdapter implements RichTextEditorAdapter<EditorJsOutputData> {
  readonly capabilities: EditorCapabilities = { blockTypes: new Set(supportedBlockTypes), inlineMarks: new Set(['bold', 'italic', 'color', 'highlight', 'link', 'mention']) };
  ...
  private toBlock(block: LorekitBlock): EditorJsBlock {
    switch (block.type) {
      case 'heading': return { type: 'header', data: { level: block.level, text: LorekitDocumentCodec.inlineToHtml(block.content) } };
      case 'paragraph': return { type: 'paragraph', data: { text: LorekitDocumentCodec.inlineToHtml(block.content) } };
      case 'list': return { type: 'list', data: { style: block.style, meta: { start: block.start }, items: block.items.map(item => this.toListItem(item)) } };
      case 'quote': return { type: 'quote', data: { text: LorekitDocumentCodec.inlineToHtml(block.content), caption: LorekitDocumentCodec.inlineToHtml(block.caption) } };
