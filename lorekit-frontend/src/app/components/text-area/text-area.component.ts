...
@Component({
  ...
  imports: [FormsModule, NgClass, HistoryFieldDirective],
  template: `
  <div class="flex flex-col">
    <label class="block text-xs font-medium mb-1" [style.color]="labelColor() || null">{{ label() }}</label>
    <textarea [historyRead]="historyRead()" [historyField]="historyField()" [placeholder]="placeholder()" [ngClass]="height()" class="w-full rounded-lg bg-zinc-940 px-3 py-2 text-sm ring-1 ring-zinc-900 transition focus:outline-none focus:ring-1 focus:ring-zinc-100" [(ngModel)]="value"></textarea>
  </div>
  `,
