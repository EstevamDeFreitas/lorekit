...
@Component({
  selector: 'app-world-configured-fields',
  imports: [MobilePinchZoomDirective, NgStyle, InputComponent, EditorComponent, ComboBoxComponent, TextAreaComponent, DynamicImageFieldComponent],
  template: `
          ...
          <div class="config-grid-item" [ngStyle]="itemStyle(item)">
            ...
            @if (metadata) {
              @if (metadata.source === 'schema') {
                @switch (metadata.key) {
                  @case ('concept') {
                    <app-text-area [label]="metadata.label" [(value)]="world().concept" (valueChange)="requestSave.emit()"></app-text-area>
                  }
                }
              }
              @else {
                ...
                @if (dynamicTemplate) {
                  ...
                  @else if (dynamicTemplate.isEditorField) {
                    <div class="editor-wrapper">
                      ...
                      <app-editor [entityId]="getDynamicFieldValue(dynamicTemplate.id).id" [docTitle]="dynamicTemplate.name" entityTable="World" [entityName]="world().name" [document]="getDynamicFieldValue(dynamicTemplate.id).value || ''" (saveDocument)="onDynamicEditorSave($event, dynamicTemplate.id)" class="rounded-lg border border-zinc-800 bg-zinc-925 h-full overflow-y-auto scrollbar-dark"></app-editor>
                    </div>
                  }
  ...
  `,
  styleUrl: './world-configured-fields.component.css',
})
export class WorldConfiguredFieldsComponent {
  ...
  activeLayout = this.uiFieldConfigService.getResolvedConfig('World', null);
  ...
  ngDoCheck(): void {
    const currentWorldId = this.world().id ?? '';
    if (currentWorldId !== this.lastWorldId) {
      this.lastWorldId = currentWorldId;
      this.loadLayout();
  ...
  gridStyle(): Record<string, string> {
    ...
    const rows = this.visibleItems.reduce((acc, item) => Math.max(acc, item.row + item.height), 4);
    const minHeight = Math.max(180, rows * rowHeight);
  ...
  private loadLayout(): void {
    const layout = this.uiFieldConfigService.getResolvedConfig('World', this.world().id || null);
    const catalog = this.uiFieldConfigService.getCatalog('World');
    ...
    if (!this.visibleItems.length) {
      const fallback = this.uiFieldConfigService.getResolvedConfig('World', null);
  ...
  private loadDynamicState(): void {
    ...
    const templates = this.dynamicFieldService.getDynamicFields('World');
    ...
    const values = this.dynamicFieldService.getEntityDynamicFieldsValues('World', worldId);
