...
@Component({
            ...
            @if (dynamicTemplate) {
              @if (dynamicTemplate.isEditorField) {
                <div class="editor-wrapper">
                  ...
                  <app-editor [entityId]="getDynamicFieldValue(dynamicTemplate.id).id" [docTitle]="dynamicTemplate.name" entityTable="Organization" [entityName]="organization().name" [document]="getDynamicFieldValue(dynamicTemplate.id).value || ''" (saveDocument)="onDynamicEditorSave($event, dynamicTemplate.id)" class="rounded-lg border border-zinc-800 bg-zinc-925 h-full overflow-y-auto scrollbar-dark"></app-editor>
                </div>
              }
  ...
  `,
  styleUrl: './organization-configured-fields.component.css',
})
export class OrganizationConfiguredFieldsComponent {
  ...
  activeLayout = this.uiFieldConfigService.getResolvedConfig('Organization', null);
  ...
  private lastOrganizationId = '';

  ngDoCheck(): void {
    const currentOrganizationId = this.organization().id ?? '';
    if (currentOrganizationId !== this.lastOrganizationId) {
      this.lastOrganizationId = currentOrganizationId;
      this.loadLayout();
  ...
  saveDynamicValues(): void {
    ...
    this.dynamicSaveTimeout = setTimeout(() => {
      ...
      this.dynamicFieldService.saveEntityDynamicFieldsValues('Organization', organizationId, values);
    }, 220);
  }

  private loadLayout(): void {
    const layout = this.uiFieldConfigService.getResolvedConfig('Organization', this.organization().id || null);
    const catalog = this.uiFieldConfigService.getCatalog('Organization');
    ...
    if (!this.visibleItems.length) {
      const fallback = this.uiFieldConfigService.getResolvedConfig('Organization', null);
  ...
  private loadDynamicState(): void {
    ...
    const templates = this.dynamicFieldService.getDynamicFields('Organization');
    ...
    const values = this.dynamicFieldService.getEntityDynamicFieldsValues('Organization', organizationId);
