...
@Component({
  selector: 'app-location-configured-fields',
            ...
            @if (dynamicTemplate) {
              ...
              @else if (dynamicTemplate.isEditorField) {
                <div class="editor-wrapper">
                  ...
                  <app-editor [entityId]="getDynamicFieldValue(dynamicTemplate.id).id" [docTitle]="dynamicTemplate.name" entityTable="Location" [entityName]="location().name" [document]="getDynamicFieldValue(dynamicTemplate.id).value || ''" (saveDocument)="onDynamicEditorSave($event, dynamicTemplate.id)" class="rounded-lg border border-zinc-800 bg-zinc-925 h-full overflow-y-auto scrollbar-dark"></app-editor>
                </div>
              }
  ...
  `,
  styleUrl: './location-configured-fields.component.css',
})
export class LocationConfiguredFieldsComponent {
  ...
  activeLayout = this.uiFieldConfigService.getResolvedConfig('Location', null);
  ...
  ngDoCheck(): void {
    ...
    if (currentLocationId !== this.lastLocationId) {
      this.lastLocationId = currentLocationId;
      this.loadLayout();
  ...
  private loadLayout(): void {
    const layout = this.uiFieldConfigService.getResolvedConfig('Location', this.location().id || null);
    const catalog = this.uiFieldConfigService.getCatalog('Location');
    ...
    if (!this.visibleItems.length) {
      const fallback = this.uiFieldConfigService.getResolvedConfig('Location', null);
  ...
  private loadDynamicState(): void {
    ...
    const templates = this.dynamicFieldService.getDynamicFields('Location');
    ...
    const values = this.dynamicFieldService.getEntityDynamicFieldsValues('Location', locationId);
